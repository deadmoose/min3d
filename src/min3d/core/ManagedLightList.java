...
public class ManagedLightList 
{
	...
	// Array of which GL_LIGHTS are enabled, where index corresponds to GL_LIGHT[index]
	...
	public void add(Light $light)
	{
		...
		if (_lights.size() > Renderer.NUM_GLLIGHTS) throw new Error("Exceeded maximum number of Lights");
	...
	Light getLightByGlIndex(int $glIndex) /*package-private*/
	{
		for (int i = 0; i < _lights.size(); i++)
		{
			...
			if (_lightToGlIndex.get(light) == $glIndex) return light;
		}
		return null;
	}
